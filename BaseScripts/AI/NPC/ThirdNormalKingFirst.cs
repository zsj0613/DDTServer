    ...
    public class ThirdNormalKingFirst : ABrain
    {
        ...
        private void FireSkill()
        {
            foreach (Player player in Game.GetAllFightPlayers())
            {
                if (player.IsLiving)
                {
                    m_AllSkill.Add(((PVEGame)Game).CreatePhysicalObj(player.X, player.Y, "Dici", "game.crazytank.assetmap.Dici", "1", 1, 1, 0, 1));
                }
            }
        ...
        private void CreateEstate()
        {
            ...
            foreach (SimpleNpc npc in child)
            {
                ...
                m_AllSkill.Add(((PVEGame)Game).CreatePhysicalObj(npc.X - 10, npc.Y, "Attack", "game.crazytank.assetmap.Buff_powup", "1", 1, 1, 0, 1));
            }
        ...
        private void ReliveBoss()
        {
            ...
            (Game as PVEGame).ParamLiving.CaeateAction = "raise";
