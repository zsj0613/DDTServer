...
namespace Fighting.Server
{
	public class FightServer : BaseServer
	{
		...
		public static void CreateInstance(FightServerConfig config)
		{
			...
			{
                FileInfo logConfig = new FileInfo("Fightlogconfig.xml");
                if (!logConfig.Exists)
				{
					ResourceUtil.ExtractResource(logConfig.Name, logConfig.FullName, Assembly.GetAssembly(typeof(FightServer)),true);
				}
