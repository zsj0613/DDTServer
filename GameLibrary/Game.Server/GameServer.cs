...
namespace Game.Server
{
	public class GameServer : BaseServer
	{
		...
		public static void CreateInstance(GameServerConfig config)
		{
			...
			{
                FileInfo logConfig = new FileInfo("Gamelogconfig.xml");
                if (!logConfig.Exists)
				{
					ResourceUtil.ExtractResource(logConfig.Name, logConfig.FullName, Assembly.GetAssembly(typeof(GameServer)),true);
				}
		...
		protected void LoadShopLimitCount(object sender)
		{
			try
			{
				...
				if (GameProperties.LimitShopState && DateTime.Now.ToString("hh:mm") == GameProperties.RefreshLimitShopCount)
				{
