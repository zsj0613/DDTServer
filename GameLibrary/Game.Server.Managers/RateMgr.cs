...
namespace Game.Server.Managers
{
	public class RateMgr
	{
		...
		{
            if(GameServer.Instance.Configuration!= null)
		...
		{
			int result;
			if (GameProperties.SEIZENPC)
			{
				List<RateInfo> temp = new List<RateInfo>();
				foreach (RateInfo ri in RateMgr.m_RateInfos)
				{
					if (ri.Type > 1000 && RateMgr.IsValid(ri))
					{
						temp.Add(ri);
					}
				}
				if (temp.Count > 0)
				{
					int index = RateMgr.m_random.Next(0, temp.Count);
					if ((float)RateMgr.m_random.Next(0, 1000) <= temp[index].Rate * 10f)
					{
						result = temp[index].Type;
						return result;
					}
				}
			}
