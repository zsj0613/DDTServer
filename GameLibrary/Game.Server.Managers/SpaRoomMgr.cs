...
namespace Game.Server.Managers
{
	public class SpaRoomMgr
	{
		...
		private static bool LoadSpaPropertyFromDb()
		{
			...
			string[] _SpaPubRoomCount = GameProperties.COUNT_SPA_PUBROOM.Split(new char[]
			{
				','
			});
			string[] _SpaPubRoomLoginPay = GameProperties.LOGIN_PAY_SPA_PUBROOM.Split(new char[]
			{
				','
			});
			string[] _SpaPubRoomMinLimit = GameProperties.PLAYER_TIMELIMIT_SPA_PUBROOM.Split(new char[]
			{
				','
			});
			string[] _SpaPubRoomPlayerMaxCount = GameProperties.PLAYER_MAXCOUNT_SPA_PUBROOM.Split(new char[]
			{
				','
			});
			SpaRoomMgr.priRoomInit_MinLimit = GameProperties.INIT_TIME_SPA_PRIROOM;
			SpaRoomMgr.priRoomContinue_MinLimit = GameProperties.CONTINUE_TIME_SPA_PRIROOM;
			string[] pubRoomsServerIdTemp = GameProperties.SERVERID_SPA_PUBROOM.Split(new char[]
			{
				'|'
			});
			...
			{
				if (SpaRoomMgr.log.IsErrorEnabled)
				{
			...
			else
			{
				...
				else
				{
					...
					else
					{
						...
						{
							if (GameProperties.INIT_TIME_SPA_PRIROOM <= 0 || GameProperties.CONTINUE_TIME_SPA_PRIROOM <= 0)
							{
								if (SpaRoomMgr.log.IsErrorEnabled)
								{
									SpaRoomMgr.log.Error("SpaPriRoomTime Init Wrong");
								}
								result = false;
							}
							else
		...
		{
			SpaRoomMgr._locker.AcquireWriterLock();
			SpaRoom result;
			try
			{
		...
		{
			SpaRoom[] list = null;
			SpaRoomMgr._locker.AcquireReaderLock();
			try
			{
