...
using CrossFighting.Server.GameObjects;
...
namespace CrossFighting.Server
{
	...
	{
        private new static LogProvider log => CrossFightServer.log;
		private RSACryptoServiceProvider m_rsa;
		private CrossFightServer m_svr;
		...
		{
			ServerClient[] clientlist = CrossFightServer.Instance.GetAllClients();
		...
		public void HandleLogin(GSPacketIn pkg)
		{
			...
			if (temp.Length == 2)
			{
				...
				base.Strict = false;
                if (temp[1] != CrossFightServer.Instance.Configuration.Key)
                {
		...
		public ServerClient(CrossFightServer svr) : base(new byte[20480], new byte[20480])
		{
			this.m_svr = svr;
			base.Strict = false;
		}
