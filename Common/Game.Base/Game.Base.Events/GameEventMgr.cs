...
namespace Game.Base.Events
{
	public sealed class GameEventMgr
	{
				...
				if (type.IsClass)
				{
					...
					for (int j = 0; j < methods.Length; j++)
					{
						...
						if (myAttribs.Length != 0)
						{
							...
							{
                                    LogProvider.Default.Error("Error registering global event. Method: " + type.FullName + "." + mInfo.Name, ex);
							}
						}
					}
				}
		...
		private static void AddHandler(object obj, RoadEvent e, RoadEventHandler del, bool unique)
		{
			...
			{
                LogProvider.Default.Error("Failed to add local event handler!", ex);
			}
		}
		...
		public static void RemoveHandler(object obj, RoadEvent e, RoadEventHandler del)
		{
			...
			{
                LogProvider.Default.Error("Failed to remove local event handler!", ex);
			}
		}
		...
		public static void RemoveAllHandlers(bool deep)
		{
			if (deep)
			{
				...
				{
                    LogProvider.Default.Error("Failed to remove all local event handlers!", ex);
				}
			}
